...
session = HTTP(
    api_key="uxtjCW3g1CZdkzcPRY",
    api_secret="s79DSaA4DivvXl1c1YellLTltD4wm9QzdrrK",
    demo=True,
    recv_window=10000
)
        ...
        if positions:
            for position in positions:
                ...
                if size > 0:
                    ...
                    if size == float(MonedasMinimasDelTick):
                        RecomprasEjecutadas = 1
                    elif size == float(MonedasMinimasDelTick) * 2:
                        RecomprasEjecutadas = 2
                    elif size >= float(MonedasMinimasDelTick) * 4:
                        RecomprasEjecutadas = 3
                    elif size >= float(MonedasMinimasDelTick) * 8:
                        RecomprasEjecutadas = 4
                    elif size >= float(MonedasMinimasDelTick) * 16:
                        RecomprasEjecutadas = 5
                    elif size >= float(MonedasMinimasDelTick) * 32:
                        RecomprasEjecutadas = 6
                    elif size >= float(MonedasMinimasDelTick) * 64:
                        RecomprasEjecutadas = 7
                    elif size >= float(MonedasMinimasDelTick) * 128:
...
def webhook():
    ...
    if datos:
        ...
        ticker = partes[1].replace('BYBIT:', '').replace('.P', '') 
...
def vigilar_posicion():
    global TickATrabajar, MonedasEnLaPosicion, RecomprasEjecutadas, PrecioDeEntradaEnLaPosicion, DireccionDeLaPosicion
